...
# Gitea部署脚本
# 用法: ./scripts/deploy-gitea.sh [staging|production] [version]
...
case $ENVIRONMENT in
    staging)
        SERVER_HOST=${GITEA_STAGING_HOST:-"staging.example.com"}
        SERVER_USER=${GITEA_STAGING_USER:-"deploy"}
        SERVER_PATH=${GITEA_STAGING_PATH:-"/opt/fastapi-project"}
        GITEA_REGISTRY=${GITEA_REGISTRY:-"gitea.example.com"}
        ;;
    production)
        SERVER_HOST=${GITEA_PRODUCTION_HOST:-"production.example.com"}
        SERVER_USER=${GITEA_PRODUCTION_USER:-"deploy"}
        SERVER_PATH=${GITEA_PRODUCTION_PATH:-"/opt/fastapi-project"}
        GITEA_REGISTRY=${GITEA_REGISTRY:-"gitea.example.com"}
        ;;
esac

log_info "开始Gitea部署到 $ENVIRONMENT 环境"
...
log_success "Gitea部署到 $ENVIRONMENT 环境完成！"
